...
import { createAccountant } from "@/lib/staff-actions";
...
export function StaffForm({ schools }: { schools: SchoolRef[] }) {
  ...
  return (
    ...
    >
      <h2 className="mb-3 text-sm font-semibold">Nouveau comptable</h2>
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        >
          {pending ? "Création…" : "Créer le comptable"}
        </button>
        {state?.error && <span className="text-sm text-red-600">{state.error}</span>}
        {state?.success && <span className="text-sm text-emerald-600">{state.success}</span>}
      </div>
      <p className="mt-2 text-xs text-neutral-400">
        Le comptable pourra se connecter immédiatement avec cet email et ce mot de
        passe. Communiquez-les-lui de façon sûre.
      </p>
    </form>
  );
}
